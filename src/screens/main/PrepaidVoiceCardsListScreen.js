...
const PrepaidVoiceCardsListScreen = () => {
  ...
  const fetchCards = async () => {
    try {
      setLoading(true);
      console.log('🔄 Fetching prepaid voice cards...');
      const data = await prepaidVoiceCardsAPI.getActiveCards();
      console.log('✅ Prepaid voice cards fetched:', data);

      if (data.success) {
        setCards(data.data || []);
      }
    } catch (error) {
      console.error('❌ Error fetching prepaid voice cards:', error);
      Alert.alert('Error', 'Failed to load prepaid voice packs. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleBuyCard = (card) => {
    navigation.navigate('PrepaidVoiceCardPayment', { card });
  };
...
export default PrepaidVoiceCardsListScreen;
